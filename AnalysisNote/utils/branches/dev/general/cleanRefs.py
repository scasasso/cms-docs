...
class cleanRefs:
    ...
    def __init__(self, tag, baseDir, verbose):
        ...
        self._rules =[ ('VOLUME',re.compile('[A-G]\d'),'Volume with serial number'),
    ...
    def getRefList(self):
        ...
        file =  self._baseDir + self._tag + '_temp.aux'
        f = io.open(file,'r')
        refs = []
        for line in f:
    ...
    def getRefs(self):
        """Open the bibfile and scan for "@artType{citation,", where citation matches one we are looking for. Extract the fields
           """
        file = self._baseDir + 'auto_generated.bib'
        ...
        for key in self._refs:
            if not key in self._bib:
                ...
                if 'AUTHOR' in self._bib[key][1].keys():
                    ...
                    if (nauthors > 1 and nauthors < 15) and etal and ~collab:
    ...
    def printLog(self):
        ...
        file =  self._baseDir + self._tag + '_temp.blg'
        f = io.open(file,'r')
        for line in f:
