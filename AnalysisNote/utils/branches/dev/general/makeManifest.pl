...
my $VERSION = sprintf "%d.%03d", q$Revision: 144582 $ =~ /(\d+)/g;
my $verbose;
...
#
   ...
   # \RCS$Revision: 144582 $
   # \RCS$Date: 2012-08-22 21:43:44 +0200 (Wed, 22 Aug 2012) $
   # \RCS$HeadURL: svn+ssh://emiglior@svn.cern.ch/reps/tdr2/utils/branches/dev/general/makeManifest.pl $
   # \RCS$Id: makeManifest.pl 144582 2012-08-22 19:43:44Z alverson $
   ...
   if ($pos)
   {
       ...
       {
           print ">>> -------------------------------------------------------------------------- <<<\n";
           print ">>>!!!<<< The abstract appears to have an unescaped TeX comment character (%). No comments are allowed within the abstract! <<<\n";
           print ">>> -------------------------------------------------------------------------- <<<\n";
       }
   }
   else
...
#      &insert_datafield("859",$record,'cds.support@cern.ch');
      ...
      if ($thumbnails) 
      {
          if ($^O eq "MSWin32")
          {
              $ENV{'PATH'} = $ENV{'ProgramFiles'}."\\gs\\gs9.02\\bin;$ENV{'PATH'}";
              $convertCmd = $ENV{'ProgramFiles'}."\\ImageMagick-6.7.5-Q16\\convert.exe";
          }
          else
          {
              $convertCmd = "convert";
