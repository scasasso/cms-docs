...
__version__ = "$Revision: 122115 $"
#$HeadURL: svn+ssh://emiglior@svn.cern.ch/reps/tdr2/utils/branches/dev/general/pas-bib.py $
#$Id: pas-bib.py 122115 2012-05-15 18:12:08Z alverson $
...
class DocList:
    ...
    def __init__(self, verbosity, remoteUser, noteType, overwrite, export):
    ...
    def getDocInfoFromCDS(self, tag):
        ...
        if (opts.verbose > 1): print "Retrieving...",
        xobj = urlopen(url)
        if (opts.verbose > 2): print "parsing...",
        dom = parse(xobj)
...
def main(argv):
    ...
    pas = DocList(opts.verbose, opts.remoteUser, opts.type, opts.overwrite, opts.export)
